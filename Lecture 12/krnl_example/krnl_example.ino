...
struct k_t *p1, *p2, *p3, *s1, *s2, *s3;
unsigned long starttime1, starttime2,  starttime3;

void task1()
{	
	k_set_sem_timer(s1, 400);
...
void task3()
{	
	k_set_sem_timer(s3, 800);
	
	while (1) {
    k_wait(s3, 800);
    unsigned long current_time3 = millis();
    Serial.print("Task3 -> Time since last run: ");
    Serial.println(current_time3-starttime3);
		k_eat_msec(200);
    current_time3 = millis();
    Serial.print("Task3 -> Time to finish run: ");
    Serial.println(current_time3-starttime3);
    starttime3 = millis();
	}
}
